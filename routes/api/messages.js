...
// Get blocked field value
router.get('/getBlock/query', (req, res) => {
    ...
    Conversation.aggregate([
        ...
        .match({ recipients: { $all: [{ $elemMatch: { $eq: user1 } }] } })
        .match({ recipients: { $all: [{ $elemMatch: { $eq: user2 } }] } })
        .project({
            'recipientObj.password': 0,
            'recipientObj.__v': 0,
            'recipientObj.date': 0,
    ...
    Message.aggregate([
        ...
        .exec((err, messages) => {
            if (err) {
                ...
                // if(messages[0].conversation){
                //     messages.push({blocked: messages[0].conversation[0].blocked})
                //     console.log(messages[messages.length - 1].blocked)
                // }                
                res.send(messages);
            }
        });
